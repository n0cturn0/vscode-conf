...
<body>
               ...
               <tr class="noHover">
                  ...
                  <?php if (!empty($numero_lacre)) { ?>
                     <th colspan="<?php echo (!empty($numero_lacre)) ? '4' : '8'; ?>">
                        <input id="validar_<?php echo $value[$i]; ?>" type="radio" name="acao_lacre[<?php echo  $value[$i]; ?>]" value="validar" class="acao_lacre">
                        <label for="validar_<?php echo $value[$i]; ?>">Validar</label>
                        <input id="alterar_<?php echo $value[$i]; ?>" checked type="radio" name="acao_lacre[<?php echo  $value[$i]; ?>]" value="alterar" class="acao_lacre">
                        <label for="alterar_<?php echo $value[$i]; ?>">Alterar</label>

                        <input id="justificar_<?php echo $value[$i]; ?>" name="acao_lacre[<?php echo  $value[$i]; ?>]" type="radio" name="acao_lacre[<?php echo  $value[$i]; ?>]" value="justificar" class="acao_lacre">
                        <label for="justificar_<?php echo $value[$i]; ?>">Jus<u>tific</u>ar</label>
                  ...
                  <th class="th_lacre" colspan="<?php echo (!empty($numero_lacre)) ? '4' : '4'; ?>">
                     <span id="labelLacre_<?php echo $i; ?>">Número do lacre:</span>
                     <input type="number" required="" minlength="7" maxlength="7" id="lacre_<?php echo $i; ?>" min="1" name="numero_lacre[<?php echo $value[$i]; ?>]" value="<?php echo $numero_lacre; ?>" class="numero_lacre ">
                  </th>
   ...
   <script type="text/javascript">
      $(".acao_lacre").click(function() {
         var numeroLacreInput = $(this).closest('tr').find('.numero_lacre');
         var numeroLacreLabel = $(this).closest('tr').find('label[for="' + numeroLacreInput.attr('id') + '"]');
         $(this).closest('tr').find('.th_lacre').show();
         $(this).closest('tr').find('#opcoesAssinatura').hide();
         numeroLacreInput.show().val('<?php echo $numero_lacre; ?>');
         if ($(this).val() === 'validar') {
            numeroLacreInput.attr('readonly', true);
            numeroLacreInput.css('background-color', '#e0e0e0');
         } else if ($(this).val() === 'justificar') {
            numeroLacreInput.attr('readonly', true);
            numeroLacreInput.css('background-color', '#e0e0e0');

            // Oculta o campo e define o valor como "0000001"
            numeroLacreInput.val('0000001');

            // Bloqueia o campo "validar" novamente
            $(this).closest('tr').find('.acao_lacre[value="validar"]').prop('checked', false);

            // Altera o rótulo para "Sem lacre"
            $(this).closest('tr').find('.th_lacre').hide();
            $(this).closest('tr').find('#opcoesAssinatura').show();
         } else {
            numeroLacreInput.removeAttr('readonly');
            numeroLacreInput.css('background-color', 'field');

            // Exibe o campo e redefine o valor para o original


            // Habilita o campo "validar" novamente
            $(this).closest('tr').find('.acao_lacre[value="validar"]').removeAttr('disabled');

            // Restaura o rótulo para "Número do lacre"
            numeroLacreLabel.text('Número do lacre');
         }
      });
