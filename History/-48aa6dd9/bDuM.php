...
class HomeController extends Controller
{
    ...
    public function deparacurriculo($id=Null)
    {
        $vagas =  DB::table('vagas')
        ->join('curriculos', 'curriculos.idvaga', '=', 'vagas.id')
        ->select('vagas.vaga','curriculos.nome','curriculos.curriculo','curriculos.id')
        ->where('vagas.id', '=', $id)
        ->get();
        
       return view('vagas.showvaga',['vagas' => $vagas]);
    }

    public function apagavaga($id=NULL)
    {
        $vaga = Vaga::query()->findOrFail($id);
        $vaga->delete();
        return redirect()->route('admcolaboradores')->with('success', 'Vaga Excluída com sucesso!');
    }
