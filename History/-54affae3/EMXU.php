...
abstract class CommonITILObject extends CommonDBTM
{
    ...
    public function prepareInputForUpdate($input)
    {
        ...
        if (
            isset($input["status"])
            && !in_array($input["status"], array_merge(
                $this->getSolvedStatusArray(),
                $this->getClosedStatusArray()
            ))
        ) {
    ...
    public function showGroupsAssociated($type, $canedit, array $options = [])
    {
        ...
        if (($itemtype == 'Ticket')
            ...
            if (
                $options['_tickettemplate']->isMandatoryField("_groups_id_" . $typename)
    ...
    public function showSuppliersAssociated($type, $canedit, array $options = [])
    {
        ...
        if (($itemtype == 'Ticket')
            ...
            if (
                $options['_tickettemplate']->isMandatoryField("_suppliers_id_" . $typename)
    ...
    public function showUsersAssociated($type, $canedit, array $options = [])
    {
        ...
        if (($itemtype == 'Ticket')
            ...
            if (
                $options['_tickettemplate']->isMandatoryField("_users_id_" . $typename)
    ...
    public function showActorAddForm(
        ...
        $types = ['group' => __('Group'),'user' => __('User')];


        
         /**
         * Regra para validar o chamado e apresentar a validação do lacre
         * O tipo do ativo tem a obrigatoriedade de ser um item do tipo computador e classificado como Desktop
         * Se for Computador do tipo desktop a estiver no grupo laboratório, só será possível atribuir o chamado para um usuário, não um grupo.
         */
        $plugin = new Plugin();
        ...
        $grupos_result  = $DB->query("SELECT group_id FROM glpi_computers_lacre_group");
        $state_result  = $DB->query("SELECT group_id FROM glpi_computers_lacre_status");
        $grupos = $grupos_result->fetch_all(MYSQLI_ASSOC);
        $state = $state_result->fetch_all(MYSQLI_ASSOC);
        $grupos_str     = implode(',', array_column($grupos, 'group_id'));
        $state_str      = implode(',', array_column($state, 'status_id'));
        $verifica_grupo = $DB->query("SELECT * FROM glpi_groups_tickets WHERE tickets_id='$id_current' AND groups_id IN ($grupos_str)");
        $verifica_tipo  = $DB->query("SELECT itemtype, items_id, tickets_id FROM glpi_items_tickets JOIN glpi_computers ON glpi_computers.id = glpi_items_tickets.items_id 
        WHERE tickets_id='$id_current' 
        AND itemtype='Computer' 
        AND glpi_computers.computertypes_id= 1
        ");
        
        
    //     $verifica_state = $DB->query("SELECT * FROM glpi_states JOIN glpi_computers_lacre_status ON glpi_states.id = glpi_computers_lacre_status.status_id
    //    WHERE glpi_states.id IN ($state_str)");




        $verifica_status  = $DB->query("SELECT itemtype, items_id, tickets_id FROM glpi_items_tickets JOIN glpi_computers ON glpi_computers.id = glpi_items_tickets.items_id 
        WHERE tickets_id='$id_current' 
        AND itemtype='Computer' 
        AND glpi_computers.computertypes_id= 1
        AND glpi_computers.states_id IN ($state_str)
        ");

        

        
        $CheckValidade = $DB->query("SELECT  status  FROM glpi_computer_lacre_hystori where id_ticket='$id_current' AND (status = 1 OR status = 2 OR status = 3)");
      
        $contatipo      = ($verifica_tipo->num_rows);
        $conta_grupo    = ($verifica_grupo->num_rows);  
        $CountValidade  = ($CheckValidade->num_rows);
        $conta_status   = ($verifica_status->num_rows);

       /**Estrutura de controle para exibição ou não dos grupos */
        if ($contatipo > 0 AND $conta_grupo != 0 ) {
        if ($withgroup) { $types=['user' => __('User')]; } 
        }else{ $types = ['group' => __('Group'),'user' => __('User') ]; }
        
        if ($CountValidade > 0){
        if ($withgroup) { $types=[ 'group'=> __('Group')];}
        }

        if ($conta_status > 0){
        if ($withgroup) { $types = ['group' => __('Group'),'user' => __('User') ]; }
        }
    ...
    public function showSolutionForm($knowbase_id_toload = 0)
    {
        ...
        if ($canedit) {
            SolutionType::dropdown([
                'value' => $this->getField('solutiontypes_id'),
                'rand' => $rand_type,
                'entity' => $this->getEntityID()
            ]);
        } else {
        ...
        if ($conta_grupo === 0) {
            ...
            if ($plugin->isActivated('psglacre')) {
                ...
                $verifica_tipo = $DB->query("SELECT itemtype, items_id, tickets_id FROM glpi_items_tickets JOIN glpi_computers ON
        glpi_computers.id = glpi_items_tickets.items_id WHERE tickets_id='$id_current' AND itemtype='Computer' 
        AND glpi_computers.computertypes_id= 1");
                $contatipo = ($verifica_tipo->num_rows);
                $camputadores = array();
                if ($contatipo > 0) {
                    ...
                    if (!empty($computadores)) {
                        ...
                        if ($rowcount == 0) {
                            ...
                            echo "<br><a href='" . $CFG_GLPI["root_doc"] . "/plugins/psglacre/front/maketab.form.php?ticket_id=" . $id_current . "' class='vsubmit' style='margin-top: 10px;'>" . _sx('button', 'Lacre') . "</a>";
                            echo '<br>';
    ...
    public function getUsedRequestTypeBetween($date1 = '', $date2 = '')
    {
        ...
        if ($DB->numrows($result) >= 1) {
            while ($line = $DB->fetch_assoc($result)) {
                ...
                $tmp['link'] = Dropdown::getDropdownName(
                    'glpi_requesttypes',
