   ...
   static function itemAddForm(Ticket $ticket, $options = [])
   {
      ...
      if (!empty($params['items_id'])) {
         ...
         if (empty($ticket)) {
            ...
            //Verifica se está no grupo pertinente
            ...
            if ($grupos_result) {
               ...
               $verifica_grupo = $DB->query("SELECT * FROM glpi_groups_tickets WHERE tickets_id='$ticket_id' AND groups_id IN ($grupos_str)");
            ...
            if ($conta_grupo === 0) {
               ...
               $verifica_tipo = $DB->query("SELECT itemtype, items_id, tickets_id,glpi_computer_lacre_hystori.lacre_number, glpi_computer_lacre_hystori.id FROM glpi_items_tickets 
         JOIN glpi_computers ON
         glpi_computers.id = glpi_items_tickets.items_id 
         WHERE tickets_id='$ticket_id' 
         AND itemtype='Computer' 
         AND glpi_computers.computertypes_id=1 
         AND glpi_computer_lacre_hystori.status IN (
  SELECT MAX(glpi_computer_lacre_hystori.status)
  FROM glpi_computer_lacre_hystori
  WHERE glpi_computer_lacre_hystori.computer_id = glpi_computers.id
)
AND glpi_computer_lacre_hystori.id = (
  SELECT MAX(glpi_computer_lacre_hystori.id)
  FROM glpi_computer_lacre_hystori
  WHERE glpi_computer_lacre_hystori.computer_id = glpi_computers.id
  AND glpi_computer_lacre_hystori.status = (
    SELECT MAX(glpi_computer_lacre_hystori.status)
    FROM glpi_computer_lacre_hystori
    WHERE glpi_computer_lacre_hystori.computer_id = glpi_computers.id
  )
        ...
        ");
               // $verifica_tipo = $DB->query("SELECT items_id, tickets_id, glpi_computer_lacre_hystori.status, glpi_computer_lacre_hystori.lacre_number, glpi_computer_lacre_hystori.id
               // FROM glpi_items_tickets 
               // JOIN glpi_computers ON
               // glpi_computers.id = glpi_items_tickets.items_id 
               // JOIN glpi_computer_lacre_hystori ON
               // glpi_computer_lacre_hystori.computer_id = glpi_computers.id
                  ...
                  if (!empty($computadores)) {
                     ...
                     if ($rowcount == 0) {
                        ...
                        if ($plugin->isActivated('psglacre')) {
                           echo "<br><a href='" . $CFG_GLPI["root_doc"] . "/plugins/psglacre/front/maketab.form.php?ticket_id=" . $ticket_id . "' class='vsubmit' style='margin-top: 10px;'>" . _sx('button', 'Lacre') . "</a>";
                           echo '<br>';
                           echo "<p id='label_lacre'>Lacre(s) não validados</p><input id='validar_lacre' type='checkbox' style='display:none'>";
                        }
                     } else {
                        
                        echo "<style> .fa-desktop { animation: piscar 1s infinite;}@keyframes piscar {0% {opacity: 1;}50% {opacity: 0;}100% {opacity: 1;}}</style>";
                        echo "<br>";
                        echo "<input type='checkbox' checked name='lacre' id='lacre'><a href='" . $CFG_GLPI["root_doc"] . "/plugins/psglacre/front/maketab.form.php?ticket_id=" . $ticket_id . "'><u><b>$rowcount Lacre(s) validado(s)</b></u><span class='fa fa-desktop' style='font-size: 15px; color: blue;'></span></a>";
                     }
                  }
               } else {
               }
         ...
         if ($data["linked_action"]) {
            ...
            switch ($data["linked_action"]) {
               ...
               case self::HISTORY_UPDATE_DEVICE:
                  ...
                  $tmp['change']  = sprintf(
                     __('Change the component %1$s: %2$s'),
                     $tmp['field'],
                     sprintf(
                        __('%1$s by %2$s'),
                        $data["old_value"],
    ...
    **/
   static function dropdownAllDevices(
      $myname,
      $itemtype,
      $items_id = 0,
      $admin = 0,
